...
ticketPriceTotal = runners.reduce((accumulator, currentRunner) => (accumulator += currentRunner.donation), 0);
console.log(ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1 - Everyone that works at Gigashots and Voolia have been disqualified for doping, so we need a list of the perpetrators!
let guiltyOfDoping = [];
guiltyOfDoping = runners.filter(runner => runner.company_name === 'Voolia' || runner.company_name === 'Gigashots')
console.log(guiltyOfDoping);

// Problem 2 - People that donated under $100 don't get a free shirt. We need a list of those who qualify.
let sufficientDonation = [];
sufficientDonation = runners.filter(runner => runner.donation >= 100)
console.log(sufficientDonation)

// Problem 3 - Give the runners cool middle names
let nicknames = ['Snarky Zinger', 'Pretty Boy', 'Lightning Catcher', 'Rabbit Tamer', 'Sketchy Lady', 'Turtle Sprinter'];
let coolMiddleNames = []
coolMiddleNames = runners.map(runner => `${runner.first_name} ${nicknames[Math.floor(Math.random()*nicknames.length)]} ${runner.last_name}`);
console.log(coolMiddleNames)